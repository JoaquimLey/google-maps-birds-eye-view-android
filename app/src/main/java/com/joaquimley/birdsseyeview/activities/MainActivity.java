...
import com.joaquimley.birdsseyeview.helpers.GoogleServicesHelper;

import java.util.LinkedList;

/**
 * Activity with GoogleMap
 */
...
public class MainActivity extends FragmentActivity {
    ...
    private void init() {
        mExampleRoute = GoogleServicesHelper.createMapRoute(new LatLng(37.783986, -122.408059),
    ...
    protected void onResume() {
        super.onResume();
        init();
        if (GoogleServicesHelper.googleServicesAvailability(this) == null) {
            setUpMapIfNeeded();
        }
    }
    ...
    /**
     * Creates a DEBUG "instance" of the map with a dummy route etc.
     */
    private void setUpMap() {
        ...
        GoogleServicesHelper.addMarker(mMap, "", mExampleRoute[0], false,
                BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE));
//        mMarker = mMap.addMarker(new MarkerOptions()
//                .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE))
//                .position(mExampleRoute[0]));

        // Create a polyline for the route.
        GoogleServicesHelper.createPolyline(mMap, mExampleRoute, Color.HSVToColor(POLYLINE_ALPHA,
                                                                new float[]{
...
//                .width(POLYLINE_WIDTH));
            ...
            public void onMapLoaded() {
                ...
                mMap.setOnCameraChangeListener(new GoogleMap.OnCameraChangeListener() {
                    ...
                    public void onCameraChange(CameraPosition position) {
                        ...
                        animateRoute(mExampleRoute);
                    }
                });

                // Animate the camera to the beginning of the route.
                GoogleServicesHelper.animateCameraToRouteBeginning(mExampleRoute, mMap, CAMERA_OBLIQUE_ZOOM, CAMERA_OBLIQUE_TILT);
            }
        });

        // Move the camera over the start position.
        GoogleServicesHelper.moveCameraToStartPosition(mMap, mExampleRoute, CAMERA_OBLIQUE_ZOOM);
    }

    public void animateRoute(LatLng[] route) {
        ...
        for (int i = 0; i < mExampleRoute.length - 1; i++) {
    ...
    class LatLngEvaluator implements TypeEvaluator<LatLng> {
        ...
        public LatLngEvaluator(LatLng startValue, LatLng endValue) {
            dLatitude = endValue.latitude - startValue.latitude;
            dLongitude = endValue.longitude - startValue.longitude;
        }

        @Override
        public LatLng evaluate(float fraction, LatLng startValue, LatLng endValue) {
            double lat = dLatitude * fraction + startValue.latitude;
            double lng = dLongitude * fraction + startValue.longitude;
